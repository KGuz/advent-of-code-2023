...
func (d Day22) PartOne(input string) string {
	bricks := d.parse(input)

	supports := d.simulate(bricks)
	essential := d.essential(supports)
	redundant := len(bricks) - len(essential)

	return strconv.Itoa(redundant)
}
...
func (d Day22) PartTwo(input string) string {
	...
	supports := d.simulate(bricks)
	essential := d.essential(supports)
	falling := d.disintegrate(essential, supports, len(bricks))

	return strconv.Itoa(falling)
}
...
func (b Brick) end() point3 {
	return b.pos.add(b.dir.mul(b.len))
}

func (self Brick) collides(other Brick) bool {
	for i := 0; i <= self.len; i++ {
		p := self.pos.add(self.dir.mul(i))

		for j := 0; j <= other.len; j++ {
			q := other.pos.add(other.dir.mul(j))
			if p == q {
				return true
			}
		}
...
func (Day22) parse(input string) []Brick {
	re := regexp.MustCompile(`\d+`)

	bricks := make([]Brick, 0)
	for _, s := range lines(input) {
		nums := transform(captures(re, s), parse)
		pos := point3{nums[0], nums[1], nums[2]}
		end := point3{nums[3], nums[4], nums[5]}

		dir := end.sub(pos)
		len := dir.l1dist(point3{0, 0, 0})
		if len != 0 {
			dir = dir.div(len)
		}

		bricks = append(bricks, Brick{pos, dir, len})
	}
	return bricks
}

func (Day22) simulate(bricks []Brick) map[int][]int {
	slices.SortFunc(bricks, func(a, b Brick) int {
		return cmp.Compare(a.pos.z, b.pos.z)
	})

	supports := map[int][]int{}
	for i := 0; i < len(bricks); i++ {
		collides := false
		for !collides && bricks[i].pos.z > 0 {
			bricks[i].pos.z--

			for j := i - 1; j >= 0; j-- {
				if bricks[i].collides(bricks[j]) {
...
func (Day22) essential(support map[int][]int) map[int]bool {
	essential := map[int]bool{}
	for _, sup := range support {
		if len(sup) == 1 {
			essential[sup[0]] = true
		}
	}
	return essential
}

func (Day22) disintegrate(essential map[int]bool, supports map[int][]int, brickCount int) int {
	falling := 0
	for i := range essential {
		disintegrated := map[int]bool{i: true}
		for j := i + 1; j < brickCount; j++ {

			if val, ok := supports[j]; ok {
				if all(val, func(elem int) bool { _, ok := disintegrated[elem]; return ok }) {
					disintegrated[j] = true
				}
			}

		}
		falling += len(disintegrated) - 1
	}
	return falling
}
