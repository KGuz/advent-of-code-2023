	...
	for _, ranges := range maps {
		...
		for n, seed := range seeds {
			for _, rmap := range ranges {
				if rmap.src <= seed && seed < rmap.src+rmap.len {
					next[n] = rmap.dst + (seed - rmap.src)
					break
				}
			}
		}
...
func (Day05) parse(input string) ([]int, [][]RangeMap) {
	...
	for _, part := range parts[1:] {
		...
		for i := 0; i < len(nums); i += 3 {
			ranges = append(ranges, RangeMap{nums[i], nums[i+1], nums[i+2]})
		}
