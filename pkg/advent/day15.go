...
type Day15 struct {
	...
	old lens with the new lens: remove the old lens and put the new lens in its
	...
	focusing power of all of the lenses. The focusing power of a single lens is
...
func (d Day15) PartTwo(input string) string {
	...
	boxes := [256][]string{}      // stack of labels
	focal := make(map[string]int) // label to focal length

	for _, s := range sequence {
		label, last := 0, len(s)-1

		if s[last-1] == '=' {
			label = d.hash(s[:last-1])
			focal[s[:last-1]] = int(s[last] - '0')

			if n := slices.Index(boxes[label], s[:last-1]); n != -1 {
				boxes[label][n] = s[:last-1]
			} else {
				boxes[label] = append(boxes[label], s[:last-1])
			}
		} else {
			label = d.hash(s[:last])

			if n := slices.Index(boxes[label], s[:last]); n != -1 {
				boxes[label] = slices.Delete(boxes[label], n, n+1)
			}
		}
	}
