...
func (d Day24) PartOne(input string) string {
	hailstones := d.parse(input)
	fmt.Printf("hailstones: %v\n", hailstones)

	lines := transform(hailstones, func(hs HailStone) [2]float64 {
		p, v := hs.pos, hs.vel
		q := p.add(v)
		// p.y = a*p.x + b
		// q.y = a*q.x + b
	...
	for i := 0; i < len(lines); i++ {
		pa, pb, pp, pv := lines[i][0], lines[i][1], hailstones[i].pos, hailstones[i].vel
		for j := i + 1; j < len(lines); j++ {
			qa, qb, qp, qv := lines[j][0], lines[j][1], hailstones[j].pos, hailstones[j].vel

			// where do lines cross?
			// y = pa * x + pb
			// y = qa * x + qb

			if pa == qa && pb != qb { // parallel lines
				continue
			}
			...
			// does cross point is before or after starting position of hailstones
			isvalid := func() bool {
				if pv.x > 0 {
					if x < pp.x {
						return false
...
func (Day24) parse(input string) []HailStone {
	re := regexp.MustCompile(`-?\d+`)
	hailstones := []HailStone{}
	for _, s := range lines(input) {
		nums := transform(captures(re, s), parse)
		hailstones = append(hailstones, HailStone{
			pos: point3f{float64(nums[0]), float64(nums[1]), float64(nums[2])},
			vel: point3f{float64(nums[3]), float64(nums[4]), float64(nums[5])},
		})
	}
	return hailstones
}
