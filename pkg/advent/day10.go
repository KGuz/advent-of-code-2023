...
func (d Day10) PartOne(input string) string {
	...
	return strconv.Itoa(len(pipes) / 2)
}
...
func (d Day10) PartTwo(input string) string {
	...
	d.visualize(graph, loop, enclosed)
...
func (d Day10) traverse(graph [][]byte, i, j int) []Pipe {
	pipe := d.next(graph, Pipe{i, j, i, j})
	loop := make([]Pipe, 0)

	for graph[pipe.ci][pipe.cj] != 'S' {
		loop = append(loop, pipe)
		pipe = d.next(graph, pipe)
	}
	return slices.Insert(loop, 0, pipe)
}
...
func (d Day10) next(graph [][]byte, pipe Pipe) Pipe {
	for _, dir := range orthogonal() {
		...
		curr, next := graph[pipe.ci][pipe.cj], graph[ni][nj]

		if d.pipesConnect(curr, next, dir) {
			return Pipe{ni, nj, pipe.ci, pipe.cj}
		}
	}
