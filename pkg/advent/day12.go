...
func (d Day12) PartOne(input string) string {
	springRows, damagedGroups := d.parse(input)
	sum := 0

	for n := 0; n < len(springRows); n++ {
		springs, damaged := springRows[n], damagedGroups[n]
		memo := make(map[HotSpringsState]int, len(springs))
		sum += d.combinations(springs, damaged, &memo)
	}
...
type HotSpringsState struct {
	springs    string
	groupCount int
}

func HotSprings(springs []byte, damaged []int) HotSpringsState {
	return HotSpringsState{string(springs), len(damaged)}
}
...
func (Day12) unfold(springs *[]byte, damaged *[]int) {
	tempSprings := make([]byte, 0, len(*springs)*5+4)
	tempDamaged := make([]int, 0, len(*damaged)*5)
	for n := 0; n < 5; n++ {
		if n != 0 {
			tempSprings = append(tempSprings, '?')
		}
		tempSprings = append(tempSprings, *springs...)
		tempDamaged = append(tempDamaged, *damaged...)
	}
	*springs = tempSprings
	*damaged = tempDamaged
}

func (d Day12) combinations(springs []byte, damaged []int, memo *map[HotSpringsState]int) int {
	if len(damaged) == 0 {
		if len(springs) == 0 {
			return 1
		}
	...
	if val, ok := (*memo)[HotSprings(springs, damaged)]; ok {
		return val
	}
	...
	switch springs[0] {
	case '.':
		combo = d.combinations(springs[1:], damaged, memo)
	case '?':
		springs[0] = '#'
		combo = d.combinations(springs, damaged, memo)
	...
	case '#':
		count := damaged[0]
		if len(springs) == count && !slices.Contains(springs[:count], '.') {
			combo = d.combinations(springs[count:], damaged[1:], memo)
			break
		}

		if len(springs) > count && springs[count] != '#' && !slices.Contains(springs[:count], '.') {
			temp := springs[count]
			springs[count] = '.'
	...
	(*memo)[HotSprings(springs, damaged)] = combo
	return combo
}
